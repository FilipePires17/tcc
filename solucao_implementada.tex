...
\subsubsection{Banco de dados}
    ...
    Foi utilizado o MySQL, um Sistema de Gerenciamento de Banco de Dados Relacional baseado em SQL, para armazenar e gerenciar os dados do sistema. O centro do aplicativo são os locais, logo, foi criada a tabela ``Locales'' que armazena os dados gerais que compõem um determinado local. Sendo que cada local pode possuir horários de funcionamento durante a semana, sendo armazenado na tabela ``Schedules'', com uma relação de um para um. Os locais também podem ter fotos, sendo armazenados na tabela ``Photos'', com uma relação de um para muitos.
    ...
    Na imagem abaixo, está descrita a estrutura do banco de dados do aplicativo Encontre na UFMS.
...
\end{figure}
    ...
    A Figura 2 foi feita utilizando o DBeaver [1], que é uma ferramenta de gerenciamento de banco de dados, foi utilizada uma função que monta graficamente a estrutura do banco de dados utilizado.
